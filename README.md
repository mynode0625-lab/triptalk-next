...
| `/api/reviews` | 후기 읽기(GET) · 남기기(POST, 로그인 필요). Supabase 미설정이면 섹션 자체가 안 보임 |
| `/admin/reviews` | 후기 관리 — 감추기·되돌리기·삭제. 관리 열쇠를 입력해야 열림 |
| `/api/admin/reviews` | 관리 목록(GET) · 상태 변경(PATCH) · 삭제(DELETE). `x-admin-key` 헤더 필요 |
...
부적절한 후기는 `/admin/reviews` 에서 **감추거나 지웁니다.** 감추기는 되돌릴 수 있고
삭제는 되돌릴 수 없으니, 먼저 감춰두고 판단하는 편이 안전합니다.

**관리자는 한 사람**입니다. 아이디와 비밀번호로 로그인하며, 계정은 데이터베이스가 아니라
서버 설정에 둡니다 — 운영자가 한 명이면 회원 테이블도 비밀번호 재설정 흐름도 필요 없습니다.

```
ADMIN_ID=...           # 서버 전용, NEXT_PUBLIC_ 금지
ADMIN_PASSWORD=...     # openssl rand -base64 18
```

로그인하면 서버가 서명한 HttpOnly 쿠키를 2시간 발급하고, 이후 관리 API 는 그 쿠키만
봅니다. 비밀번호가 매 요청에 실려 다니지 않고, 자바스크립트가 읽을 수 없는 쿠키만
남으므로 브라우저에 값이 저장돼 다음 사람이 이어받는 일도 없습니다.

> **로그인 계정으로 관리자를 가리지 않는 이유** — 신한 SOL 연동 키가 없어 로그인이 데모로
> 돌고, 데모 세션은 방문자마다 임시 발급되며 이메일은 모두 같은 값입니다. "이 계정만
> 관리자" 라고 걸면 데모로 들어온 누구나 통과합니다. 연동이 끝나면 `session.sub` 허용
> 목록으로 바꾸는 편이 낫습니다 — 그래야 누가 지웠는지가 남습니다.
