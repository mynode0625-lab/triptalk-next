...
- [x] ~~저장소를 Public으로 전환~~ — **Private 유지.** Vercel Hobby 는 *개인 계정* 소유의
      Private 저장소를 그대로 배포한다 (제약은 조직 소유 저장소에만 적용). 이 저장소는
      개인 계정 소유이므로 공개할 이유가 없다
...
**그대로 둔 것 — 세션을 브라우저 저장소에 두는 방식.** 데모 단계의 의도된 설계이고
(`lib/auth/session.ts` 주석에 명시), 서버는 액세스 토큰을 클라이언트로 내려보내지 않고
정규화한 프로필만 돌려준다. 실제 서비스로 갈 때 HttpOnly 쿠키로 옮긴다.
